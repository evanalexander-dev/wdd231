export async function running(ip, user, pass) {
    try {
        ...
        const response = await fetch(`http://${ip}:5000/webcast/running`, { headers });
        if (response.ok) {
...
export async function start(ip, user, pass) {
    try {
        const encodedAuth = btoa(`${user}:${pass}`);
        const headers = { "Authorization": `Basic ${encodedAuth}` };
        const response = await fetch(`http://${ip}:5000/webcast/start`, { headers });
        if (response.ok) {
            return await response.json();
        } else {
            throw Error(await response.text());
...
export async function stop(ip, user, pass) {
    try {
        const encodedAuth = btoa(`${user}:${pass}`);
        const headers = { "Authorization": `Basic ${encodedAuth}` };
        const response = await fetch(`http://${ip}:5000/webcast/stop`, { headers });
        if (response.ok) {
            return await response.json();
        } else {
            throw Error(await response.text());
...
export async function restart(ip, user, pass) {
    try {
        const encodedAuth = btoa(`${user}:${pass}`);
        const headers = { "Authorization": `Basic ${encodedAuth}` };
        const response = await fetch(`http://${ip}:5000/webcast/restart`, { headers });
        if (response.ok) {
            return await response.json();
        } else {
            throw Error(await response.text());
...
export async function getLogs(ip, user, pass) {
    try {
        const encodedAuth = btoa(`${user}:${pass}`);
        const headers = { "Authorization": `Basic ${encodedAuth}` };
        const response = await fetch(`http://${ip}:5000/webcast/status`, { headers });
        if (response.ok) {
            return await response.json();
        } else {
            throw Error(await response.text());
...
export async function getSchedule(ip, user, pass) {
    try {
        const encodedAuth = btoa(`${user}:${pass}`);
        const headers = { "Authorization": `Basic ${encodedAuth}` };
        const response = await fetch(`http://${ip}:5000/webcast/upcoming`, { headers });
        if (response.ok) {
            return await response.json();
        } else {
            throw Error(await response.text());
...
export async function getSettings(ip, user, pass) {
    try {
        const encodedAuth = btoa(`${user}:${pass}`);
        const headers = { "Authorization": `Basic ${encodedAuth}` };
        const response = await fetch(`http://${ip}:5000/webcast/settings`, { headers });
        if (response.ok) {
            return await response.json();
        } else {
            throw Error(await response.text());
...
export async function postSettings(ip, user, pass, settings) {
    try {
        ...
        const response = await fetch(`http://${ip}:5000/webcast/settings`, {
            method: "POST",
